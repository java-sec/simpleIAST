...
public class HookTransformer implements ClassFileTransformer {
    ...
    public byte[] transform(ClassLoader loader, String className, Class<?> classBeingRedefined, ProtectionDomain protectionDomain, byte[] classfileBuffer) {
        ...
        try {
            ...
            if (LogTool.isDebugEnabled()) {
                LogTool.error(ErrorType.TRANSFORM_ERROR, "DoTransform error", throwable);
            }
            return classfileBuffer;
        }
     ...
     * 构建类图
     */
//    private void buildClassGraph(ClassReader classReader) throws ClassNotFoundException {
//        String className = classReader.getClassName();
//        ClassGraph classGraph = context.getClassGraph();
//        //加入到类图中
//        ClassData classData = new ClassData(className);
//        classData.setAccess(classReader.getAccess());
//        classData.buildAncestors(classReader.getInterfaces(), classReader.getSuperName());
//        classGraph.addNode(classData);
//        for (String ancestor : classData.getAncestors()) {
//            classGraph.addEdge(classData, new ClassData(ancestor));
//        }
//
//        ClassLoader classLoader = this.getClass().getClassLoader();
//        ClassUtils.buildAllAncestors(classData.getAncestors(), classLoader, classGraph);
//    }
